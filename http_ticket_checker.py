...
def get_file(resource):
    ...
    if ticket_id and ticket_id is not "":
        valid_ticket = validate_ticket(ticket_id, resource_id)
...
def validate_ticket(ticket_id, resource):
    if ticket_id_pattern.match(ticket_id):
        ticket = get_ticket(ticket_id)
        ticket = parse_ticket(ticket)
        if ticket:
            resources, type_, user_id = ticket
            # strip everything up to, and including, "..uuid:" from the resource-id's
            resources = map(lambda r: r.split(":")[-1], resources)
            valid = (app.config['CONTENT_TYPE'] == type_ and resource in resources)
            if valid:
                return True
            else:
                return False
...
def parse_ticket(ticket):
    if ticket:
        if not False in [ticket.has_key(key) for key in ["resources", "type", "userIdentifier"]]:
